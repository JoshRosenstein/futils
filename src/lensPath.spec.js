...
describe('lensPath', () => {
  ...
  describe('over', () => {
    it('applies function to the value of the specified object property', () => {
      expect(over(lensPath(['d']), x=>x+1, testObj)).toEqual({a: [{b: 1}, {b: 2}], d: 4})
      expect(over(lensPath(['a', 1, 'b']), x=>x+1, testObj)).toEqual({a: [{b: 1}, {b: 3}], d: 3})
      expect(over(lensPath([]), toPairs, testObj)).toEqual( [['a', [{b: 1}, {b: 2}]], ['d', 3]])
    })
    it('applies function to undefined and adds the property if it doesn\'t exist', () => {
      expect(over(lensPath(['X']), x=>x, testObj)).toEqual({a: [{b: 1}, {b: 2}], d: 3, X: undefined})
